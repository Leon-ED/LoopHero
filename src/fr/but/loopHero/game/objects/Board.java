...
public class Board {
	...
	public void createLoop(int numberOfCells) {
		...
		for (Cell cell : listCellsLoop) {
			...
			for (int x = -1; x <= 1; x = x + 2) {
				int newI = i + x;
				if (!(listCellsLoop.contains(boardMatrix[newI][j])
						|| boardMatrix[newI][j].type().name().equalsIgnoreCase("RoadSide")))
					boardMatrix[i + x][j].setType(new RoadSide("Wasteland", Color.DARK_GRAY.brighter()));
			}
			for (int y = -1; y <= 1; y = y + 2) {
				int newJ = j + y;
				if (!(listCellsLoop.contains(boardMatrix[i][newJ])
						|| boardMatrix[i][newJ].type().name().equalsIgnoreCase("RoadSide")))
	...
	public void getSpawnableEntity(HashMap<Mobs, Cell> dict_mob_cell) {
		// Pour les phases suivantes, boucler sur tous le tableau et vï¿½rif les impacts
		// sur les tiles d'a cotï¿½ et ajouter une liste de carte qui se fait impacter
		// par les tiles d'a cotï¿½.
	}
	...
	public ArrayList<Cell> getlistCellsLoop() {
		return listCellsLoop;
	}
}