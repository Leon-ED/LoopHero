...
public abstract class Mobs {
	...
	public Mobs(String string, int percentageOfSpawn, int healthPoint, double strenght, double speed, Color color,
			double dropChance, Cell cell, ArrayList<Droppable> MOBS_DROPPABLE_ITEMS, double evadeChance) {
		this.name = Objects.requireNonNull(string);
		this.percentageOfSpawn = percentageOfSpawn;
		this.maxHealth = (int) ((int) (healthPoint * LoopHeroGameData.LEVEL)
				* (1 + (LoopHeroGameData.LEVEL - 1) * 0.02));
	...
	public int takeDamage(int damages, CombatEffects usedSpecialEffect, GameGraphics graphics,
		...
		if (usedSpecialEffect == null) {
			graphics.showEffect(context, attaque, "Le mob a perdu : " + damages + " HP", Color.GREEN);
		...
		case Counter -> {
			...
			graphics.showEffect(context, attaque, "Le mob contre l'attaque, le héro perds : " + damages + " HP",
					Color.BLUE);
			return -damages;
		}
		...
		case Vampirism -> {
			graphics.showEffect(context, attaque,
					"Le mob a utilisé le vampirisme et obtient :" + damages + " HP en plus !", Color.BLUE);
			heal(damages);
			return 0;
		}
	...
	public boolean equals(Object o) {
		if (!(o instanceof Mobs))
			return false;
		Mobs mob = (Mobs) o;
		return this.getPos()[0] == mob.getPos()[0] && this.getPos()[1] == mob.getPos()[1]
				&& healthPoint == mob.healthPoint && color.equals(mob.color);
